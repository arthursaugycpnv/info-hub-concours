...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!rate_limit('comment', 5, 600)) {
        $erreurCommentaire = 'Trop de commentaires envoyés. Réessayez dans 10 minutes.';
    }

    $auteur  = isLoggedIn() ? $_SESSION['user_nom'] : trim($_POST['auteur'] ?? '');
    $contenu = trim($_POST['contenu'] ?? '');
    if (!$erreurCommentaire && $auteur && $contenu) {
                    ...
                    <ul class="list-group list-group-flush">
                        <?php foreach ($commentaires as $c): ?>
                            <li class="list-group-item">
                                ...
                                <p class="mb-0 small comment-body"><?= nl2br(htmlspecialchars($c['contenu'])) ?></p>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                <?php else: ?>
...
(function(){
    ...
    document.querySelectorAll('.comment-body').forEach(function(el){
        ...
        if(el.textContent.length <= MAX) return;
        var truncated = el.textContent.substring(0, MAX).trimEnd() + '…';
        el.textContent = truncated;
        var btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'btn btn-link btn-sm p-0 d-block mt-1';
